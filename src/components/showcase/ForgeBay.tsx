...
import { PartWheel } from "./PartWheel";
...
function StockChip({ shape, active }: { shape: BlankShape; active?: boolean }) {
  ...
  return (
    <span className="animate-chip-float relative block" aria-hidden="true">
      ...
      <span
        className={cn(
          "animate-chip-pulse pointer-events-none absolute inset-0 border-2 border-arc/50",
          round,
        )}
      />
      {/* ground shadow (depth) */}
...
function DragBlank({
  ...
  const zone = useCallback(() => {
    ...
    return {
      ...
      cy: b.top + b.height * 0.46,
      r: Math.min(b.width, b.height) * 0.24,
      b,
    };
  }, [bayRef]);
  ...
  const move = useCallback(
    (e: PointerEvent) => {
      const b = bayRef.current!.getBoundingClientRect();
      setPos({
        x: e.clientX - b.left - offset.current.x,
        y: e.clientY - b.top - offset.current.y,
      });
      const z = zone();
    ...
    <>
      ...
      <div
        className={cn(
          ...
          near
            ? "scale-110 border-arc opacity-100"
            : "scale-100 border-arc/25 opacity-70",
        )}
      />
  ...
  return (
    <section id="showcase" className="relative py-4 sm:py-8">
      <div className="container-x">
        {/* ── STAGE: part wheel (left) + forge bay / 3D viewer (right) ── */}
        <div className="grid gap-4 lg:grid-cols-[minmax(0,260px)_minmax(0,1fr)] lg:gap-6">
          {/* scroll-wheel picker — a rail beside the viewer */}
          <div className="order-2 h-[336px] lg:order-1 lg:h-[min(64svh,600px)]">
            <PartWheel
              parts={showcaseParts}
          ...
          {/* ── THE FORGE BAY (3D viewer) ── */}
          <div
            ref={bayRef}
            data-forge-bay
            className="relative order-1 select-none overflow-hidden rounded-3xl border border-line bg-gradient-to-b from-panel-2/70 to-ink lg:order-2"
          ...
          >
            ...
            <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between p-5 sm:p-7">
              <div>
                <span className="eyebrow">
                  <span
                    className="h-1.5 w-1.5 rotate-45 bg-arc"
                    aria-hidden="true"
                  />
                  {selected.category}
                </span>
              ...
              <div className="pointer-events-auto flex items-center gap-2">
                ...
                {selected.model && (
                  <a
                    href={`/view/${selected.id}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    title="Open just the 3D model on its own page"
                    aria-label="Open just the 3D model on its own page"
                    className="hidden h-10 items-center gap-1.5 rounded-full border border-line bg-panel/50 px-4 font-mono text-[0.6rem] uppercase tracking-[0.16em] text-muted backdrop-blur transition-colors hover:border-arc/50 hover:text-fg sm:inline-flex"
                  >
                    <Box className="h-3.5 w-3.5" /> View 3D only
                  </a>
                )}
                <button
                  type="button"
              ...
              <div className="pointer-events-none absolute inset-0 grid place-items-center px-6">
                <div className="max-w-sm rounded-2xl border border-line bg-panel/70 px-8 py-7 text-center backdrop-blur">
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-muted">
                    The live model for the {selected.name.toLowerCase()} is
                    being tessellated from CAD. Request it and we&rsquo;ll send
                    the 3D and the drawing.
                  </p>
                </div>
              </div>
            )}
            ...
            <AnimatePresence>
              {formed && (
                ...
                >
                  <p className="mb-3 max-w-md text-sm leading-relaxed text-muted">
                    {selected.blurb}
                  </p>
                  <dl className="flex flex-wrap items-end gap-x-7 gap-y-3">
                    ...
                    {selected.model && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="pointer-events-auto ml-auto inline-flex items-center gap-1.5 rounded-full border border-line bg-panel/60 px-4 py-2 font-mono text-[0.6rem] uppercase tracking-[0.16em] text-muted backdrop-blur transition-colors hover:border-arc/50 hover:text-fg sm:hidden"
                      >
                        <Box className="h-3.5 w-3.5" /> View 3D only
                      </a>
                    )}
                    <a
                      href="/contact"
                      className={cn(
                        "pointer-events-auto inline-flex items-center gap-1.5 rounded-full bg-arc px-4 py-2 font-mono text-[0.6rem] uppercase tracking-[0.16em] text-ink transition-colors hover:bg-arc-bright",
                        selected.model ? "sm:ml-auto" : "ml-auto",
                      )}
                    >
        ...
        {/* ── SELECTED PART — full-width detail ── */}
        <div className="mt-6 rounded-2xl border border-line bg-panel/40 p-6 sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-2xl">
              ...
              <h4 className="mt-2 font-display text-2xl font-bold tracking-tightest text-fg sm:text-3xl">
                {selected.name}
              </h4>
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-3 lg:justify-end">
              <span
                className={cn(
                  "inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 font-mono text-[0.55rem] uppercase tracking-[0.18em]",
                  selected.model
                    ? "border-arc/40 bg-arc/[0.08] text-arc"
              ...
              >
                <span
                  className={cn(
                    ...
                    selected.model ? "bg-arc" : "bg-faint",
                  )}
                  aria-hidden="true"
                />
                {selected.model
              ...
              {selected.model && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 rounded-full border border-line bg-panel/60 px-3 py-1.5 font-mono text-[0.55rem] uppercase tracking-[0.16em] text-muted transition-colors hover:border-arc/50 hover:text-fg"
                >
                  <Box className="h-3.5 w-3.5" /> View 3D only
                </a>
              )}
          ...
          <dl className="mt-7 grid grid-cols-2 gap-x-8 gap-y-5 border-t border-line pt-6 sm:grid-cols-4">
            {SPEC_KEYS.map((k) => (
              <div key={k}>
                <dt className="font-mono text-[0.55rem] uppercase tracking-[0.2em] text-faint">
                  {SPEC_LABELS[k]}
                </dt>
                <dd className="mt-1 font-display text-base font-semibold text-fg">
                  {selected[k]}
                </dd>
              </div>
            ))}
